...
#include <std_msgs/Float32.h> // Float32 메시지를 사용하기 위해 추가
#include <std_msgs/Bool.h>
#include <std_msgs/Int32.h>   // Int32 메시지 사용
...
// === 브로드캐스트 명령 상수: 헤더에 없으면 직접 정의 ===
#ifndef CMD_ROBOT_MONITOR_BC_ON
#define CMD_ROBOT_MONITOR_BC_ON   0x4C  // 76
#endif
#ifndef CMD_PNT_IO_MONITOR_ON
#define CMD_PNT_IO_MONITOR_ON     0x55  // 85
#endif
...
ros::Publisher l_wheel_encoder_pub;
std_msgs::Int32 l_wheel_en;
ros::Publisher r_wheel_encoder_pub;
std_msgs::Int32 r_wheel_en;
...
static bool torque_off_flag;

// 브로드캐스트 활성화 여부 플래그
static bool g_mdui_broadcast_enabled = false;
...
void InitMotorParameter(void)
{
    switch(byCntInitStep)
    {
        ...
        case SETTING_PARAM_STEP_PID_PPR:
        {
            ...
            // === 브로드캐스트 활성화: MDUI 사용시 시도, 실패 대비는 폴백 분기로 처리 ===
            g_mdui_broadcast_enabled = false;
            if (robotParamData.use_MDUI == 1) {
                // PID_PNT_IO_MONITOR 브로드캐스트 ON
                uint8_t cmd = (uint8_t)CMD_PNT_IO_MONITOR_ON;
                PutMdData(PID_COMMAND, robotParamData.nIDMDUI, &cmd, 1);

                // PID_ROBOT_MONITOR 브로드캐스트 ON
                cmd = (uint8_t)CMD_ROBOT_MONITOR_BC_ON;
                PutMdData(PID_COMMAND, robotParamData.nIDMDUI, &cmd, 1);

                g_mdui_broadcast_enabled = true; // 명령 전송 시도 완료: 이후 요청 루틴에서 생략
            }
...
void RequestRobotStatusTask(void)
{
    ...
    switch(byCntComStep)
    {
        case 0:
        {
            // PID_PNT_MAIN_DATA를 모든 장치에 요청
            req_pid = PID_PNT_MAIN_DATA; //PID 210
            PutMdData(PID_REQ_PID_DATA, ID_ALL, (const uint8_t *)&req_pid, 1); // ID_ALL
            byCntComStep = 1;
            break;
        }

        case 1: // 모니터링 데이터 요청 (브로드캐스트 미사용 시에만)
        {
            if (!g_mdui_broadcast_enabled && robotParamData.use_MDUI == 1) {
                req_pid = PID_ROBOT_MONITOR2; // PID 224, 전압/상태
                PutMdData(PID_REQ_PID_DATA, ID_ALL, (const uint8_t *)&req_pid, 1);
            }
            byCntComStep = 2;
            break;
        }

        case 2: // 배터리 데이터 요청 (브로드캐스트 미사용 시에만)
        {
            if (!g_mdui_broadcast_enabled && robotParamData.use_MDUI == 1) {
                req_pid = PID_ROBOT_MONITOR; // PID 253, 배터리 퍼센트
                PutMdData(PID_REQ_PID_DATA, ID_ALL, (const uint8_t *)&req_pid, 1);
            }
            byCntComStep = 3;
            break;
...
void torqueControlCallback(const std_msgs::Bool& torque_status)
{
    torque_off_flag = torque_status.data;
    if(torque_off_flag == 1){
        ROS_INFO("Torque_OFF");
        PID_PNT_TQ_OFF_t pid_pnt_tq_off, *p;
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber torque_off_sub = nh.subscribe("torque_off", 10, torqueControlCallback);
    int16_t *pGoalRPMSpeed;
    ...
    l_wheel_encoder_pub = nh.advertise<std_msgs::Int32>("mdh80_l_en",10); //add
    r_wheel_encoder_pub = nh.advertise<std_msgs::Int32>("mdh80_r_en",10); //add
...
void PubMDRobotMessage1(void)
{
    ...
    l_wheel_encoder_pub.publish(l_wheel_en);   //add
    r_wheel_encoder_pub.publish(r_wheel_en);   //add
}
